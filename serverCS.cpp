...
#include <fstream>
#include "string"
#define MYPORT "22472" // the port users will be connecting to
...
string comp(string coursecode, string query) //•	https://www.youtube.com/watch?v=LiyMDucaq2k
{
    ...
    for (int i = 0; i < 5; i++)
    {
        if (coursecode == obj[i].code) // check for given conditions
        {
            if (query == "Credit")
            {
...
int main()
{
    cout << "The ServerCS is up and running using UDP on port 22472." << endl;
    ...
    while (true)
    {
        ...
        {
            cout << "The ServerCS received a request from the Main Server about the " << query << " of " << courseCode << endl;
